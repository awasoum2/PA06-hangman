    ...
    if request.method == 'GET':
        ...
        while not state['done']:
            ...
            if len([x for x in state['word'] if x not in state['guesses']])==0:#"all the letters in the word have been guessed"
                ...
                state['response']="You have correctly guessed the word %s" %state['word']# print('you win!')

            elif state['guesses_left']==0:
                state['done']=True
                state['response']="You are out of guesses. The word is %s" % state['word']# print('you lost')#"set done to be true and tell the user they lost!"
            else:
                # print_word(word,guessed_letters)
                word_so_far=hangman_app.get_word_so_far(state['word'],state['guesses'])
                state['word_so_far']=word_so_far
                state['response']="Please guess again"
                letter = request.form['guess']
